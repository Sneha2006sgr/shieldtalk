...
export default function HQDashboard() {
  ...
  const approve = async (id) => {
    ...
    try {
      ...
      showToast('Approved! Link: ' + window.location.origin + r.data.activationLink);
      fetchAll();
    } catch (e) {
  ...
  const toggleMember = (id) => {
    setNewGroup((g) => ({
      ...g,
      memberIds: g.memberIds.includes(id) ? g.memberIds.filter((x) => x !== id) : [...g.memberIds, id],
    }));
  };
    ...
    <div style={{ minHeight: '100vh', background: '#020b18', color: '#e2e8f0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', background: 'rgba(10,25,47,0.9)', borderBottom: '1px solid rgba(0,255,100,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontFamily: 'monospace', fontWeight: 700, color: '#00ff64', letterSpacing: 2 }}>SHIELDTALK HQ</span>
      ...
      <div style={{ display: 'flex', gap: 4, padding: '10px 20px', flexWrap: 'wrap', borderBottom: '1px solid rgba(0,255,100,0.1)', background: 'rgba(2,11,24,0.6)' }}>
        {TABS.map((t, i) => (
          <button key={i} onClick={() => setTab(i)} style={{
            padding: '6px 14px', fontFamily: 'monospace', fontSize: 11, letterSpacing: 1,
            cursor: 'pointer', borderRadius: 4,
            background: tab === i ? 'rgba(0,100,50,0.4)' : 'transparent',
            border: '1px solid ' + (tab === i ? '#00ff64' : 'rgba(0,255,100,0.2)'),
            color: tab === i ? '#00ff64' : '#475569',
          }}>{t}</button>
        ))}
      </div>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ margin: '8px 20px', padding: '8px 14px', background: 'rgba(0,100,50,0.3)', border: '1px solid rgba(0,255,100,0.3)', borderRadius: 6, fontFamily: 'monospace', fontSize: 11, color: '#00ff64' }}>
          {toast}
        </div>
      )}
      ...
      <div style={{ padding: '16px 20px' }}>
        ...
        {tab === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 12 }}>
              {[
                { label: 'TOTAL USERS', val: stats.totalUsers || 0, icon: '👥', c: '#e2e8f0' },
                { label: 'PENDING', val: stats.pendingUsers || 0, icon: '⏳', c: '#fbbf24' },
                { label: 'ONLINE NOW', val: stats.activeUsers || 0, icon: '🟢', c: '#00ff64' },
                { label: 'ACTIVE SOS', val: stats.activeSOS || 0, icon: '🚨', c: '#f87171' },
                { label: 'STATUS', val: 'ONLINE', icon: '⬡', c: '#00ff64' },
              ].map((s, i) => (
        ...
        {/* CHAT */}
        {tab === 1 && <HQChat myId={String(user._id)} />}
          ...
          <div style={card}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'monospace', fontSize: 11 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(0,255,100,0.2)' }}>
                  ...
                  {users.map((u) => (
                    <tr key={u._id} style={{ borderBottom: '1px solid rgba(0,255,100,0.05)' }}>
                      ...
                      <td style={{ padding: '6px 8px' }}>
                        <span style={{ padding: '2px 8px', borderRadius: 3, fontSize: 10, background: u.status === 'approved' ? 'rgba(0,100,50,0.3)' : u.status === 'pending' ? 'rgba(251,191,36,0.2)' : 'rgba(248,113,113,0.2)', color: u.status === 'approved' ? '#00ff64' : u.status === 'pending' ? '#fbbf24' : '#f87171' }}>
                          {u.status?.toUpperCase()}
                        </span>
                      </td>
                      <td style={{ padding: '6px 8px' }}>
                        <span style={{ width: 8, height: 8, borderRadius: '50%', display: 'inline-block', background: u.isOnline ? '#22c55e' : '#475569' }} />
                      </td>
                      <td style={{ padding: '6px 8px' }}>
                        <select defaultValue={u.role} onChange={(e) => assignRole(u._id, e.target.value)}
                          style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(0,255,100,0.2)', color: '#e2e8f0', padding: '3px 6px', borderRadius: 4, fontFamily: 'monospace', fontSize: 10, cursor: 'pointer' }}>
                          {ROLES.map((r) => <option key={r} value={r}>{r.replace(/_/g, ' ')}</option>)}
                        </select>
                      </td>
                      <td style={{ padding: '6px 8px' }}>
                        {u.status === 'approved' && u.role !== 'hq_admin' && (
                          <button onClick={() => suspend(u._id)} style={mkBtn('#f87171')}>SUSPEND</button>
                        )}
                      </td>
                    </tr>
                  ))}
            ...
            <div style={card}>
              ...
              {newUserResult ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <p style={{ fontFamily: 'monospace', fontSize: 12, color: '#00ff64', margin: 0 }}>✓ USER CREATED</p>
                  <p style={{ fontFamily: 'monospace', fontSize: 11, color: '#94a3b8', margin: 0 }}>Share this activation link with the user:</p>
                  <div style={{ background: 'rgba(0,0,0,0.5)', padding: '8px 12px', borderRadius: 6, border: '1px solid rgba(0,255,100,0.2)', wordBreak: 'break-all' }}>
                    <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#00ff64' }}>{window.location.origin}{newUserResult.activationLink}</span>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <button onClick={createUser} style={{ ...mkBtn('#00ff64'), padding: '10px 0', width: '100%', fontSize: 12, letterSpacing: 2 }}>
                    CREATE &amp; APPROVE USER
                  </button>
                </div>
              )}
            ...
            <div style={card}>
              ...
              {groups.map((g) => (
                <div key={g._id} style={{ padding: '8px 10px', background: 'rgba(0,0,0,0.3)', borderRadius: 6, marginBottom: 6, border: '1px solid rgba(0,255,100,0.08)' }}>
                  <p style={{ margin: 0, fontFamily: 'monospace', fontSize: 12, color: '#e2e8f0' }}>👥 {g.name}</p>
        ...
        {tab === 5 && (
          <div style={card}>
            ...
            {pending.length === 0 ? (
              <p style={{ fontFamily: 'monospace', fontSize: 11, color: '#475569' }}>No pending requests.</p>
            ) : pending.map((u) => (
        ...
        {tab === 7 && (
          <div style={card}>
            ...
            {sos.length === 0 ? (
              <p style={{ fontFamily: 'monospace', fontSize: 11, color: '#475569' }}>No active SOS alerts.</p>
            ) : sos.map((a) => (
              <motion.div key={a._id}
                animate={{ borderColor: ['rgba(248,113,113,0.2)', 'rgba(248,113,113,0.7)', 'rgba(248,113,113,0.2)'] }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <p style={{ margin: 0, fontFamily: 'monospace', fontWeight: 700, color: '#f87171' }}>🚨 {a.message}</p>
